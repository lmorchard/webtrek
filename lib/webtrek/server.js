...
var WebTrek_Server = Class.extend(function () {
    ...
    return {
        ...
        broadcast: function (op, params) {
            var now = this.loop.tick; // new Date().getTime();
...
var WebTrek_Server_Client = Class.extend(function () {
    ...
    return {
        ...
        init: function (options) {
            ...
            this.last_ping_time = this.server.loop.tick; // (new Date()).getTime();
            this.pings = [];
        ...
        handleMessage: function (time, delta, msg) {
            ...
            switch (op) {
                ...
                case OPS.PING: 
                    // console.log("PING C:"+args+" S:"+this.server.loop.tick+
                    //    " D:"+(this.server.loop.tick - args));
                ...
                case OPS.PLAYER_ACTION:
                    ...
                    // console.log("PLAYER ACTION LAG " + 
                    //    ( this.server.loop.tick - remote_tick ) );
                    break;
            ...
            while (args = this.action_buffer.shift()) {
                ...
                if (this.server.loop.tick < remote_tick) { 
                    ...
                    entity.applyRemoteUpdate(remote_tick, [
                        entity.id, {angle: state.angle}, action
                    ]);
                    */
